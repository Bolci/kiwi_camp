...
class DataExporter():
    ...
    @staticmethod
    def parse_flight_informations(flight_list):
    ...
    def calculate_price(connection,bags_count):
        ...
        return sum([
            float(single_flight['base_price'] + bags_count * single_flight['bag_price'])
            for single_flight in connection
        ])
    ...
    @staticmethod
    def sort_by_parameter(flights, param = "total_price"):
        ''' sort list of flights by parameter'''
        
        return sorted(flights, key = lambda d: d[param])
    
    def prepare_output_format_file(self, flights, bags_requested,
                                   destination, origin, bags):

        output_file = list(map(lambda flight: self.prepare_single_flight(flight, bags_requested,
                                                                         destination, origin,bags), flights))
        output_file = self.sort_by_parameter(output_file)
